...
#===============================NEW CODE==================================
    def __CheckBarrierIsValid(self, CentreSquare, Direction):
        LeftSquare = CentreSquare - 10*Direction
        RightSquare = CentreSquare + 10*Direction
        if self.__CheckSquareInBounds(LeftSquare) and self.__CheckSquareInBounds(RightSquare):
            Index = self.__GetIndexOfSquare(LeftSquare)
    ...
    def __PlaceBarrier(self):
        CentreSquare = int(input("Please type the location you would like to use for the middle square of the barrier (Row then Column): "))
        BarrierValid = self.__CheckBarrierIsValid(CentreSquare, self._CurrentPlayer.GetDirection())
        ...
        if BarrierValid:
            for n in range(-1,2):
                self._Board[self.__GetIndexOfSquare(CentreSquare+n)].SetPiece(Barrier(self._CurrentPlayer,PlayerSymbol))
        else:
            print("Invalid coordinate.")
   
   
    def __CheckManhattanDistance(self, StartSquare, FinishSquare):
        StartRow = StartSquare // 10
        StartColumn = StartSquare % 10
        FinishRow = FinishSquare // 10
        FinishColumn = FinishSquare % 10
        CurrentRoute = None
        
        for r in range(StartRow, FinishRow):
            SquareReference = []
            SquareReference.append(r*10+StartColumn)
            SquareReference.append(r*10+FinishColumn)
            for n in SquareReference:
                BarrierInSquare = Square.ContainsBarrier(self._Board[self.__GetIndexOfSquare(n)])
                if BarrierInSquare:
                    CurrentRoute = False
                    print("Invalid Move")
                else:
        ...
        for c in range(StartColumn, FinishColumn):
            SquareReference = []
            SquareReference.append(10*StartRow+c)
            SquareReference.append(10*FinishColumn+c)
            for n in SquareReference:
                BarrierInSquare = Square.ContainsBarrier(self._Board[self.__GetIndexOfSquare(n)])
                if BarrierInSquare:
                    CurrentRoute = False
...
#=========================================================================
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        if not self.__CheckSquareInBounds(SquareReference):
            return False
#=============================NEW CODE=====================================
        BarrierInSquare = Square.ContainsBarrier(self._Board[self.__GetIndexOfSquare(SquareReference)])
        PieceInSquare = self._Board[self.__GetIndexOfSquare(SquareReference)].GetPieceInSquare()
        if PieceInSquare is None and not BarrierInSquare:
#==========================================================================
            if StartSquare:
                return False
            else:
